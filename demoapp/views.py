...
def send_email(contact_form_data):
    ...
    send_mail('Demo Website', email_message_format, EMAIL_HOST_USER, [EMAIL_HOST_USER],
              fail_silently=False, )
...
def contact_us(request):
    try:
        ...
        messages.error(request, 'Please configure your Email Client.')
...
def clear(request):
    form = ContactForm()
    messages.error(request, 'Fields Clear successfully.')
    return render(request, 'contact.html', {'page': 'contact', 'form': form})
